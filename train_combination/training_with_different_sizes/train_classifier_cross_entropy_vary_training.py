...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description='Training with small training sets the combination with cross-entropy loss. The code uses augmented data')
		...
		for num_train in train_numbers:
				...
				if num_train < len(train_names_noAug):
					...
					if not os.path.isfile(os.path.join(out_path_model,str(i)+'_modelo_final_'+name_architecture+'_split_random10timesEach_'+str(num_train)+'_'+str(num_val)+aug+'.h5')):
						...
						print('length:', len(train_positive_data[0][0]), len(train_negative_data[0][0]), len(val_positive_data[0][0]), len(val_negative_data[0][0]))

						if type_architecture == 0:
							# network definition
							model = create_base_network(len(positive_places_train[0]), len(positive_imagenet_train[0]), len(positive_reID_train[0]),2)
						...
						model2 = train_model(model,train_positive_data,train_negative_data,val_positive_data,val_negative_data,epochs,os.path.join(out_path_model,str(i)+'_modelo_best_'+name_architecture+'_split_random10timesEach_'+str(num_train)+'_'+str(num_val)+aug+'.h5'))
