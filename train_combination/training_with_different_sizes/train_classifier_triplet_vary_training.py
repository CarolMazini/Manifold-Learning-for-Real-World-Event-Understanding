...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description='Training with small training sets the combination with triplet loss. The code uses augmented data')
	...
	for j in range(iterations):
		...
		for num_train in train_numbers:
			...
			if num_train < len(train_names_noAug):
				...
				if not os.path.isfile(os.path.join(out_path_model,str(j)+'_modelo_final_'+name_architecture+'_split_random10timesEach_'+str(num_train)+'_'+str(num_val)+aug+'.h5')):
					...
					val_names, index_positive_val = choice_training(val_names_noAug, val_names_aug, val_samples[j][0:num_val])
					...
					tam_val = len(val_positive_data[0])
					...
					train_siamese_triplet(train_positive_data, train_negative_data, val_data, val_label, [len(positive_places_train[0]), len(positive_imagenet_train[0]), len(positive_reID_train[0])],val_batch,out_path_model, str(j)+'_modelo_'+name_architecture+'_split_random10timesEach_'+str(num_train)+'_'+str(num_val)+aug+'.h5',type_architecture)
					
					if type_architecture == 0:
						# network definition
						network = build_network([len(positive_places_train[0]), len(positive_imagenet_train[0]), len(positive_reID_train[0])])
					elif type_architecture == 1:
						# network definition
						network = build_network_deep1([len(positive_places_train[0]), len(positive_imagenet_train[0]), len(positive_reID_train[0])])
					elif type_architecture == 2:
						# network definition
					...
					path_model = 'best_'+str(j)+'_modelo_'+name_architecture+'_split_random10timesEach_'+str(num_train)+'_'+str(num_val)+aug
					...
					np.save(os.path.join(out_path_features,str(j)+'_negative_'+str(num_train)+'_'+str(num_val)+'_'+dataset+aug+'_train_'+name_architecture+'.npy'),features_negative_train)
					np.save(os.path.join(out_path_features,str(j)+'_negative_'+str(num_train)+'_'+str(num_val)+'_'+dataset+aug+'_val_'+name_architecture+'.npy'),features_negative_val)
					np.save(os.path.join(out_path_features,str(j)+'_negative_'+str(num_train)+'_'+str(num_val)+'_'+dataset+aug+'_test_'+name_architecture+'.npy'),features_negative_test)
					np.save(os.path.join(out_path_features,str(j)+'_positive_'+str(num_train)+'_'+str(num_val)+'_'+dataset+aug+'_train_'+name_architecture+'.npy'),features_positive_train)
					np.save(os.path.join(out_path_features,str(j)+'_positive_'+str(num_train)+'_'+str(num_val)+'_'+dataset+aug+'_val_'+name_architecture+'.npy'),features_positive_val)
					np.save(os.path.join(out_path_features,str(j)+'_positive_'+str(num_train)+'_'+str(num_val)+'_'+dataset+aug+'_test_'+name_architecture+'.npy'),features_positive_test)
